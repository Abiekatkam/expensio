...
export async function GET(request) {
	...
	const OR = { OR: categories?.split(',').map((category) => ({ category: { contains: category } })) };
	...
	return await checkAuthorisation(async (user) => {
		try {
			...
			const data = await prismaClient.investments.findMany({
				where,
...
export async function DELETE(request) {
	...
	return await checkAuthorisation(async (user) => {
		...
		try {
			await prismaClient.investments.delete({
				where: { id: id[0] },
			});
...
export async function PUT(request) {
	...
	return await checkAuthorisation(async () => {
		...
		try {
			await prismaClient.investments.update({
